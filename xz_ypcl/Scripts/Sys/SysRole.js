var SysRole = {
            ...
            success: function (result) {
                ...
                if (json.messCode == 200 && json.data.length > 0) {
                    ...
                    $(json.data).each(function (i, item) {
                        ...
                        Html += "<a  href=\"javascript:void(0)\" onclick=\"SysRole.Edit('" + item.RowID + "')\" title=\"编辑\">编辑</a>";
                        Html += "<a  href=\"javascript:void(0)\" onclick=\"SysRole.Del('" + item.RowID + "')\" title=\"删除\">删除</a>";
                        Html += "</td>";
                        Html += "</tr>";
                    });
                }
    ...
    Edit: function (rowid) {
        $("#showTips").fancybox({
            ...
            'onClosed': function () {
                window.location.href = '/Sys/SysRole';
            }
        });
        $("#showTips").click();
    },
    Del: function (rowid) {
        if (confirm('确定要删除吗') == true) {
            ...
            param["batch"] = false;
            $.ajax({
                url: "/Sys/DelRole", type: "post", data: param, success: function (result) {
